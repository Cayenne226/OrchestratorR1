...
```
refiner        → 重写模糊/不完整的输入描述       → gpt-4o-mini (低成本)
...
| 模糊描述 | think → refiner → executor_cheap → answer | 2 |
| 多跳推理 | think → decomposer → executor×N → synthesizer → answer | 3~5 |
| 复杂代码 | think → decomposer → executor_strong×N → critic → answer | 4~6 |
| 复杂+模糊 | think → refiner → decomposer → executor_strong×N → critic → synthesizer → answer | 5~6 |
...
| w/o refiner | 去掉 refiner Agent | prompt 优化的贡献 |
