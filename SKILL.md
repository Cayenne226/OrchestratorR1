...
```
executor (strong) → 高质量、深度推理的子任务执行    → claude-sonnet-4 (高能力)
executor (weak)   → 快速、明确的子任务执行          → gpt-4o          (中等)
decomposer        → 将复杂任务拆解为独立子任务      → gemini-2.5-pro
critic            → 评估中间结果，指出不足          → claude-sonnet-4
synthesizer       → 将多个子任务结果整合为最终答案  → gpt-4o
```

**executor 双档位**：通过 `<call type="executor" tier="strong|weak">` 在同一类型下统一调度高/低成本模型。Query rewriting 不再设独立 refiner，由编排器在 `<think>` 标签内的内部推理完成。

Agent 池是**固定基础设施**，不参与训练。Orchestrator 通过 RL 学会何时调用哪个 Agent、以什么档位调用。
...
<call type="executor" tier="weak">简单子查询</call>
<information>执行结果（系统注入）</information>

<call type="executor" tier="strong">需要深度推理的子查询</call>
<information>执行结果（系统注入）</information>
...
**关键设计**：所有 `<call>` 标签均为可选，模型通过 RL 学会哪些该用、哪些该跳过、以及 executor 用哪个档位。

### 涌现的行为谱

| 任务复杂度 | 涌现路径 | 预期轮数 |
|---|---|---|
| 简单事实题 | think → executor (weak) → answer | 1 |
| 隐含引用 | think → executor (weak) → answer（query rewriting 在 think 内完成） | 1~2 |
| 多跳推理 | think → decomposer → executor×N → synthesizer → answer | 3~5 |
| 复杂代码 | think → decomposer → executor (strong)×N → critic → answer | 4~6 |
| 复杂多步任务 | think → decomposer → executor (strong)×N → critic → synthesizer → answer | 5~6 |
...
优化策略：
- 训练阶段使用主 Worker Pool（与评估配置一致），通过 reward 中的 α·C_cost 让模型自发偏好 weak-tier 调用
- 通过 `min(α·C_cost, 0.3)` 上限避免成本项主导梯度
- 预计训练 API 成本约 $50（5000 prompts × G=8 × 平均 3 calls，主要使用 GPT-4o / Claude Sonnet 4）
...
|---|---|---|
| Full | 完整方案（4 角色） | 上界 |
| w/o critic | 去掉 critic Agent | 中间验证的贡献 |
| w/o decomposer | 去掉 decomposer Agent | 分解能力的贡献 |
| w/o synthesizer | 去掉 synthesizer Agent | 合成能力的贡献 |
| w/ refiner | 加回 refiner（反向消融） | 论证 4 角色设计是充分的，refiner 与 think 内推理冗余 |
